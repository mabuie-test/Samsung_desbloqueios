...
class AdvancedConnectionHandler:
    ...
    def current_name(self):
        return getattr(self._handler, "current_name", None)
...
class UniversalFRPManager:
    ...
    def execute_version_strategy(self, target: str) -> bool:
        if not self.connection_handler.is_connected():
            logging.error("Dispositivo não conectado para bypass FRP")
            return False
        ...
        normalized_target = (target or "auto").lower()
        if normalized_target in {"14", "android14", "android 14"}:
            return self._run_android14(profile)
        if normalized_target in {"13", "12", "android13", "android12"}:
            return self._run_modern(profile)
        if normalized_target in {"11", "10", "android11", "android10"}:
            return self._run_legacy(profile)
        if normalized_target in {"low", "baixo", "low_level", "baixo nível"}:
            return self._run_low_level(profile)

        # AUTO: decide pela versão detectada, caindo para métodos universais
        if android_version.startswith("14"):
            if self._run_android14(profile):
                return True
        elif android_version.startswith("1"):
            if self._run_modern(profile):
                return True
        else:
            if self._run_legacy(profile):
                return True

        return self._run_low_level(profile)

    # --------------------------------------------------------------
    # Internos específicos por versão/nível
    # --------------------------------------------------------------
    def _run_android14(self, profile: ChipsetProfile) -> bool:
        if self.android14.execute_advanced_bypass():
            return True
        return self._run_modern(profile)

    def _run_modern(self, profile: ChipsetProfile) -> bool:
        commands = self.operations.frp_reset_commands(profile)
        commands.extend(
            [
                "setprop persist.sys.frp.pst 0",
        ...
        self.connection_handler.send_batch(commands)
        return self._verify_frp_state()

    def _run_legacy(self, profile: ChipsetProfile) -> bool:
        commands = self.operations.frp_reset_commands(profile)
        commands.extend(
            [
                "content delete --uri content://settings/secure --where \"name='lock_screen_owner_info'\"",
                "settings put secure user_setup_complete 1",
            ]
        )
        self.connection_handler.send_batch(commands)
    ...
    def _run_low_level(self, profile: ChipsetProfile) -> bool:
        commands = [
        ...
        try:
            for partition in self.operations.security_partitions(profile):
                self.connection_handler.send(
                    f"if [ -e /dev/block/by-name/{partition} ]; then dd if=/dev/zero of=/dev/block/by-name/{partition} bs=4096 count=4; fi"
                )
        except Exception:
            logging.debug("Falha ao limpar partições de segurança em nível baixo")
        ...
        return self._verify_frp_state(force_rescue=True)

    def _verify_frp_state(self, force_rescue: bool = False) -> bool:
        try:
