...
class SamsungUnlockQtWindow(QtWidgets.QMainWindow):
    ...
    def _build_frp_tab(self) -> None:
        ...
        description = QtWidgets.QLabel("Bypass FRP por versão")
        description.setAlignment(QtCore.Qt.AlignCenter)
        layout.addWidget(description)

        buttons_row1 = QtWidgets.QHBoxLayout()
        self.frp_a14_button = QtWidgets.QPushButton("Android 14+")
        self.frp_modern_button = QtWidgets.QPushButton("Android 12/13")
        self.frp_legacy_button = QtWidgets.QPushButton("Android 10/11")
        buttons_row1.addWidget(self.frp_a14_button)
        buttons_row1.addWidget(self.frp_modern_button)
        buttons_row1.addWidget(self.frp_legacy_button)
        layout.addLayout(buttons_row1)

        buttons_row2 = QtWidgets.QHBoxLayout()
        self.frp_auto_button = QtWidgets.QPushButton("Universal/Auto")
        self.frp_low_button = QtWidgets.QPushButton("Baixo nível")
        buttons_row2.addWidget(self.frp_auto_button)
        buttons_row2.addWidget(self.frp_low_button)
        ...
        self.frp_a14_button.clicked.connect(lambda: self._bypass_frp("14"))
        self.frp_modern_button.clicked.connect(lambda: self._bypass_frp("13"))
        self.frp_legacy_button.clicked.connect(lambda: self._bypass_frp("11"))
        self.frp_auto_button.clicked.connect(lambda: self._bypass_frp("auto"))
        self.frp_low_button.clicked.connect(lambda: self._bypass_frp("low_level"))
    ...
    def _bypass_frp(self, target: str = "auto") -> None:
        def task():
            try:
                self._update_status(self.frp_status, f"Executando bypass FRP ({target})...")
                self._update_progress_bar(self.frp_progress, 20)
                self._append_connection_log(f"FRP: iniciando rotina {target}")
